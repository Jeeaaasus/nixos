  ...
  programs = {
    ...
    bash = {
      ...
      shellAliases = {
        ...
        update = ''cd ~/nix && sudo nh os switch --bypass-root-check --update --ask path:. && git add flake.lock && git commit -m "$(date +'%Y%m%d')" -m "$(nix shell nixpkgs#nvd -c nvd diff $(ls -dt /nix/var/nix/profiles/* | head -3 | tail -2 | tac) | tail +3 | awk '{gsub(/-${vars.hostname}/, sprintf("%${builtins.toString (builtins.stringLength vars.hostname)}s", "")); print}')"'';
        dev = ''nix develop'';
        gs = ''git status'';
