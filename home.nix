...

{
  ...
  home.packages =
    ...
    # Unstable packages
    (with pkgs; [
      # (app_name.overrideAttrs (oldAttrs: rec { version = "app_version"; src = fetchPypi { inherit version; pname = "app_name"; hash = "sha256-"; }; }))  # version override example
      ...
      streamlink                 # Twitch viewing CLI
      streamlink-twitch-gui-bin  # Twitch viewing GUI
  ...
  programs = {
    ...
    bash = {
      ...
      shellAliases = {
        rebuild = ''cd ~/nix && nix shell nixpkgs#nh -c nh os build path:. && read -p "> Apply the config? (press enter or abort)" && sudo nixos-rebuild switch --flake path:.'';
        update = ''cd ~/nix && nix flake update --flake . && nix shell nixpkgs#nh -c nh os build path:. && read -p "> Apply the config? (press enter or abort) " && sudo nixos-rebuild switch --flake path:. && git add flake.lock && git commit -m "$(date +'%Y%m%d')" -m "$(nix shell nixpkgs#nvd -c nvd diff $(ls -d /nix/var/nix/profiles/* | tail -2) | tail +3 | awk '{gsub(/-${vars.hostname}/, sprintf("%${builtins.toString (builtins.stringLength vars.hostname)}s", "")); print}')"'';
        dev = ''nix develop'';
        gs = ''git status'';
